...
use sc_telemetry::{Telemetry, TelemetryWorker, log};
...
pub fn new_full(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	...
	if config.offchain_worker.enabled
	{
		...
		let k = sp_keystore::SyncCryptoStore::sr25519_generate_new(
			&*keystore,
			node_template_runtime::hello_substrate::KEY_TYPE,
			Some("//Alice"),
		)
		.expect("Creating key with account Alice should succeed.");
		log::info!("In other words, public ky for Alice: {:?}", k);
	}	
