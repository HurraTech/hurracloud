...
Run easyrsa without commands for usage and commands."
	...
	while [ -n "$1" ]; do
		case "$1" in
			...
			passin) sign_opts="$sign_opts $2" shift ;;
			*) warn "Ignoring unknown command option: '$1'" ;;
		esac
		shift
	done
...
Matching file found at: "
	...
	# Sign it
	sign_req "$crt_type" "$name" batch "$sign_opts"
...
gen_crl() {
	...
	"$EASYRSA_OPENSSL" ca -utf8 -gencrl -out "$out_file_tmp" -config "$EASYRSA_SAFE_CONF" -passin env:CA_PASS || die "\
